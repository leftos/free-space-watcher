...
**PipeServer** — named pipe `FreeSpaceWatcher`, newline-delimited JSON; message contracts live in Core. ACL: SYSTEM, Administrators and INTERACTIVE. Messages: `GetStatus`, `GetConfig`, `SetConfig`, `Subscribe` (server pushes status every second, each `Alert`, and `AlertsChanged` after any acknowledge or delete), `ListAlerts`, `GetAlert`, `AckAlerts` and `DeleteAlerts` (a list of ids, or null for all), `ProcessAction` (suspend, resume or kill), `GetProcessStates`.
